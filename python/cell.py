...
class Cell:
  ...
  def __call__(self):
    if self.cache is None:
      ...
      print(f"node: {self.name} filling cache {self.cache} from {[x.name for x in (self.deps or [])]}")
    else:
      print(f"node: {self.name} using cache {self.cache} from {[x.name for x in (self.deps or [])]}")
    return self.cache
